...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background/90 p-6 shadow-2xl shadow-primary/10 backdrop-blur-xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-lg border border-transparent opacity-70 transition-opacity hover:opacity-100 hover:bg-accent/40 focus:border-ring focus:outline-none disabled:pointer-events-none data-[state=open]:bg-secondary">
        <X className="h-4 w-4" />
